...
class SimulatedRobot:
    def __init__(self, model, data) -> None:
        ...
        self.ee_offset = np.array([0.0, 0.02, -0.055])
    ...
    def solve_ik(self, ee_target_pos, ee_target_rot=None):
        """
    ...
    def compute_ik(self, ee_target_pos, ee_target_rot=None, max_attempts=10):
        ...
        for attempt in range(max_attempts):
            ...
            # Solve IK
            final_rad = self.solve_ik(ee_target_pos, ee_target_rot=ee_target_rot)
            ...
            print(f"[Control] (IK) Attempt {attempt+1}) Goal pos (radian): {command_qpos}")
            ...
            ee_target_pos += np.random.uniform(-0.01, 0.01, size=3) # threshold can be changed

        return None


    def plan_traj(self,real_robot, ee_target_pos, ee_target_rot=None, steps=100):
        """
        Plan collision free trajectory
        """
        sim_qpos = self.read_position()
        command_qpos = self.compute_ik(ee_target_pos)

        real_pwm=np.array(real_robot.read_position())
        real_pos=self._pwm2pos(real_pwm)
        self.d.qpos[:6]=real_pos
        mujoco.mj_forward(self.m,self.d)

        print("robot_pos:",real_robot.read_position())
        print("curr_qpos:",self._pos2pwm(sim_qpos))
        ...
        col_free_trajs = []
        trajs = np.linspace(real_pos, command_qpos, steps)
        for pos in trajs:
    ...
    def detect_col(self):
        ...
        for g in self.d.contact.geom:
            if g.tolist() not in [[1, 2]]:
                ...
                print("[Control] (Collision) {} and {} are in collision".format(col_source, col_target))
                COLLISION = True

        return COLLISION


    def control_pid(self, current_rad, final_rad, Kp=0.5, Ki=0.1, Kd=0.01, adjust_rate=0.5):
        '''
        ...
        self.integral_error += ee * adjust_rate
        self.derivative_error = (ee - self.previous_error) / adjust_rate
